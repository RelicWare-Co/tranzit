...
async function seedStaffOverrides() {
	...
	for (const staff of staffList) {
		...
		console.log(
			`staff override created for ${staff.userId} on ${overrideDate}`,
		);
	...
	const procedures = [
		{
			...
			eligibilitySchema: {
				fields: [
					{
						key: "hasValidLicense",
						label: "¿Tiene licencia vigente?",
						type: "boolean",
					},
					{
						key: "medicalCheck",
						label: "¿Aprobó examen médico?",
						type: "boolean",
					},
				],
			},
		...
		},
		{
			...
			eligibilitySchema: {
				fields: [
					...
					{
						key: "noPendingFines",
						label: "¿Sin comparendos pendientes?",
						type: "boolean",
					},
				],
			},
...
async function seedBookingsAndRequests(citizenUsers: CitizenUser[]) {
	...
	for (const combo of combos) {
		...
		console.log(
			`booking created: ${combo.status} for ${citizen.email} on ${slot.slotDate} ${slot.startTime}`,
		);
	}
	...
	console.log(
		`administrative series created with ${adminSlots.length} bookings`,
	);
}
...
async function seedAuditAndNotifications(adminId: string | null) {
	...
	console.log(
		`notification deliveries created: ${Math.min(requests.length, 2)}`,
	);
}
